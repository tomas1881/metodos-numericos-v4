...
export default function Jacobi() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await metodosAPI.jacobi({
        A: matriz,
        b: vector,
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6">
      <h2 className="text-2xl font-bold text-indigo-600 mb-4">🔄 Método de Jacobi</h2>
        ...
        <div className="flex gap-2">
          {[2, 3, 4, 5].map(n => (
            <button key={n} type="button" onClick={() => cambiarTamano(n)}
              className={`px-4 py-2 rounded-lg ${tamano === n ? 'bg-indigo-600 text-white' : 'bg-gray-200'}`}>
              {n} x {n}
            </button>
          ))}
        </div>
        ...
        <div className="mt-6 space-y-4">
          <div className="bg-indigo-50 p-4 rounded-lg">
            ...
            {resultado.solucion.map((val, i) => (
              <div key={i} className="flex justify-between"><span>x{i+1} =</span><span className="font-bold text-indigo-600">{val}</span></div>
            ))}
            <div className="flex justify-between mt-2"><span>Error final:</span><span>{resultado.error_final}%</span></div>
            <div><span>Convergencia:</span> {resultado.convergio ? '✅ Convergió' : '❌ No convergió'}</div>
          </div>
          ...
          {mostrarIter && resultado.iteraciones && (
            <div className="overflow-x-auto">
              <table className="min-w-full border">
                <thead className="bg-indigo-600 text-white"><tr><th>Iter</th>{resultado.solucion.map((_, i) => <th key={i}>x{i+1}</th>)}<th>Error%</th></tr></thead>
                <tbody>
